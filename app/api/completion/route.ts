...
export async function POST(request: NextRequest) {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert AI character developer. Generate detailed instructions and example conversations for an AI companion.",
        },
        {
          role: "user",
    ...
    const instructionsMatch = content.match(
      /INSTRUCTIONS:([\s\S]*?)(?=EXAMPLE_CONVERSATION:|$)/
    );
    const conversationMatch = content.match(/EXAMPLE_CONVERSATION:([\s\S]*?)$/);

    const instructions = instructionsMatch ? instructionsMatch[1].trim() : "";
    const exampleConversation = conversationMatch
